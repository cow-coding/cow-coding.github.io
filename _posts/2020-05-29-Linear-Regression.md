...
```
이 과정은 예측값을 통해서 오차를 알아내고 손실함수를 통해서 그래디언트를 입력해주는 함수를 구현해줍니다. 보통 가중치와 절편의 갱신은 훈련과정에서 이뤄지므로 이 과정에서는 그래디언트만 계산해주는 것입니다.
  ...
  def train(self, x, y, epochs=100):
    for i in range(epochs):
      for x_i, y_i in zip(x, y):
        ...
        self.b -= b_grad
```
이 부분은 데이터 셋을 통해서 가중치와 절편을 계속 갱신을 해서 최소의 손실함수 값을 찾아주게 됩니다.
