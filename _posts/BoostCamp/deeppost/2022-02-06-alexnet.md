...
1. top-1 error, top-5 error  
   ...
   - 그렇다면 이를 활용하여 top-5 error를 이해해봅시다. top-1은 상위 1개의 class가 정답 클래스와 같은지를 확인합니다. top-5 error는 모델의 예측에서 상위 5개의 클래스를 확인합니다. 만약 상위 5개의 class에 정답 class가 있는지 없는지를 확인 합니다.  
...
![](/image/boostcamp/precourse/alexnet.png){: w="600"}*AlexNet 구조 \| 출처 : NIPS2012 ImageNet Classification with Deep Convolutional Neural Networks*

- 5개의 convolution layer와 4개의 fully-connected layer로 구성된 총 8개의 학습 layer로 구성
