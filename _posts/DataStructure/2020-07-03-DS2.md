---
title: Data Structure Day 2 - Analysis of Algorithms (알고리즘 분석)
date: 2020-07-03
categories:
- Data Structure
tags:
...
## Constant Factors
상수요소들인 단순 산수연산이나 대입연산과 같은 것들은 곱하기 연산들에 큰 영향을 미치지 못한다. 예를 들어 $10n^2 + 10^5$이라는 식이 있으면 아무리 10^5이 큰 수라고 해도 $n^2$에 들어가는 수가 어떻게 변하냐에따라 더 영향이 크게된다.  
고등학교때 배운 극한에서 n을 무한대로 보낼때 최고차항만 고려해도 되는 것과 같은 이치이다.

## Big-O notation
프로그래밍에서 시간이 얼마나 걸리냐를 나타내는 시간복잡도(time complexity)를 빅오 표기법으로 나타낸다. 우리가 계산하고자하는 함수를 $f(n)$이라고 하고 비교기준 함수역할을 $g(n)$이라고하면, $f(n) \isin O(g(n))$이다. 정확하게 수식으로 나타내면 아래와 같다.
