...
1-1. 만약에 현재 데이터 범위의 중앙 값이 자신이 찾고자 하는 값보다 크면 기준 값의 왼쪽 범위는 탐색 범위에서 제외한다.
1-2. 찾고자하는 값이 중앙 값보다 작으면 기준 값의 오른쪽 범위는 탐색 범위에서 제외한다.
2. 위의 방법을 계속해서 반복하고 결국 1개의 데이터가 남았을 때, 해당값이 탐색 값과 같으면 특정 결과를 반환해준다.
3. 만약 값이 없다면 조건에 맞춰 출력해준다.
...
```
삭제는 탐색에서 플래그 상태와 밸류만 상황에 맞게 설정해주면 된다.
