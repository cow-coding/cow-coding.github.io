---
title: "Go로 만들어보는 자료구조 - 배열(Array)"
date: 2021-01-10
categories: [DataStructure, Computer Science, Go]
mathjax: true
tags:
- Data Structure
- Array
...
```
Go는 c++에서 new나 delete같은 함수가 없다. 그래서 생성자를 만들 때 포인터를 활용해서 변환된 객체를 반환해주는 코드를 작성한다. 그래서 return값이 객체 포인터 값으로 지정된다. 이 자체로 new와 같은 역할을 한다. delete는 해당 객체를 nil로 만들면된다.
