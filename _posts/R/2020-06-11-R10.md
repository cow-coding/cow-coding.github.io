...
<br>
  ...
  \\[
  median = \begin{cases} X_{((n+1/2))} \quad \text{if n is odd} \\~\\ \frac{1}{2}(X_{(n/2)}+x_{(n/2+1)}) \quad \text{if n is even}
  \end{cases}
  \\]  
...
- 산포의 측도
  ...
  \\[
  \begin{array}{cc}
  For\;X_{1}, X_{2}, ...., X_{n},\\ \\ \widehat{\sigma^2}=\frac{1}{n-1}\sum_{i=1}^{n}(X_i - \overline{X})^2
  \end{array}
  \\]  
