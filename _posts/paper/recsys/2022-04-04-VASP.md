...
- $u \in \{ 1, ... , U \}$ : user
- $i \in \{ 1, ... , I \}$ : item
- $\mathbf{x}_u = [ x_{u1}, x_{u2}, ..., x_{uI} ]^\intercal \in \mathbb{N}^I$ : user $u$ interaction history (original input)
- $\hat{\mathbf{x}}_u = [ x_{u1}, x_{u2}, ..., x_{uI} ]^\intercal \in \mathbb{N}^I$ : user $u$ predicted ratings
